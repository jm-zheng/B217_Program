...
void rs232_read_y()
{
	for(int i=0;i<11;i++)
	{
		RS232_Serial.write(read_m8_m10[i]);
...
//---------------shock_flag----------------------------------
	...
	//-----------------------------------------
		...
		if (digitalRead(count2_pin)==LOW)
		{
			...
			else if (count2_staus == 1 && count2_tag==0)
			{
				count2_staus = 0;	
				count2_counts +=1;
			}
		...
		if(second_flag < 999)
		{
			...
			//ampere_value = 0.0216*maxapval-0.0144;
			if(ampere_value < 0)
			{
				ampere_value = 0 ;
			}
		...
		else if(second_flag == 1000)
		{
			...
			myOLED.print(",");
			myOLED.print (RS232_RxBuf[7]);
			myOLED.print (RS232_RxBuf[8]);
			myOLED.print(",");
			myOLED.print(count2_counts);
			myOLED.print(",");
			myOLED.print (RS232_RxBuf[11]);
			myOLED.println (RS232_RxBuf[12]);
			...
			ZIGBEE_Serial.print(",");
			ZIGBEE_Serial.write (RS232_RxBuf[7]);
			ZIGBEE_Serial.write (RS232_RxBuf[8]);
			ZIGBEE_Serial.print(",");
			ZIGBEE_Serial.print(count2_counts);
			ZIGBEE_Serial.print(",");
			ZIGBEE_Serial.write (RS232_RxBuf[11]);
			ZIGBEE_Serial.write (RS232_RxBuf[12]);
			ZIGBEE_Serial.write(0x2c);
			ZIGBEE_Serial.print("*");
