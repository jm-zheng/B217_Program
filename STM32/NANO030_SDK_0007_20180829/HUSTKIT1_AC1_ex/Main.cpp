...
int main(void)
{
	...
	analogRead(AC_CURRENT); 

	myOLED.begin(FONT_8x16); // or FONT_6x8
	myOLED.println("HUSTKIT1 AC1");
	myOLED.setPosi(2, 0);
	myOLED.println("OFF");
	...
	while (1)
	{
		...
		{
			count=0;
			val=analogRead(AC_CURRENT);
			if(val>585)
			{
				cur=(float)(val-580)/100.0;
			}
			else
			{
				cur=0;
			...
			ZIGBEE_Serial.print("{\"OUTA1\":");
			ZIGBEE_Serial.print(cur);
			ZIGBEE_Serial.print("}");
		}

		if (ZIGBEE_Get() == 1)
		{
			if(jsonGet((char *)ZIGBEE_RxBuf,(char *)"OUTP1",buf)==1)
			{
				val=atoi(buf);
				myOLED.setPosi(4, 0);
				myOLED.println(val);
