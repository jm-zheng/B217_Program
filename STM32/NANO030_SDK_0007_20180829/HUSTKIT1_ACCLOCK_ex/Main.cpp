...
int main(void)
{
	...
	pinMode(DRV_LOCK, OUTPUT);
	digitalWrite(DRV_LOCK, HIGH);

	myOLED.begin(FONT_8x16); // or FONT_6x8
	myOLED.println("HUSTKIT1 ACCLOCK");
	...
	while (1)
	{
		...
		if (digitalRead(KEY_SELECT) == 0)
		{
			if (KEY_SELECT_Tag == 1)
			{
				...
				digitalWrite(LED_B, HIGH);
				myOLED.setPosi(2, 0);
				myOLED.println("LOCK OPEN");
				digitalWrite(DRV_LOCK, LOW);
				delay(100);
				digitalWrite(LED_B, LOW);
			}
		}
		...
		if (ZIGBEE_Get() == 1)
		{
			if(jsonGet((char *)ZIGBEE_RxBuf,(char *)"ACCLOCK",buf)==1)
			{
				val=atoi(buf);
				myOLED.setPosi(4, 0);
				myOLED.println(val);
