					...
					else
					{
						...
						for (i = 1; i < JSON_r; i++) 
						{
							if (jsoneq(JSON_ptr, &JSON_t[i], "FAN1") == 0) 
							{
								...
								val=atoi(buf);
								myOLED.println(val);
								ZIGBEE_Publish("FAN1",val);
								i++;
							} 
							else if (jsoneq(JSON_ptr, &JSON_t[i], "WIN1") == 0) 
							{
								sprintf(buf,"%.*s", JSON_t[i+1].end-JSON_t[i+1].start,JSON_ptr + JSON_t[i+1].start);
								val=atoi(buf);
								myOLED.println(val);
								ZIGBEE_Publish("WIN1",val);
								i++;
							} 
							else if (jsoneq(JSON_ptr, &JSON_t[i], "ACCLOCK") == 0) 
							{
								sprintf(buf,"%.*s", JSON_t[i+1].end-JSON_t[i+1].start,JSON_ptr + JSON_t[i+1].start);
								val=atoi(buf);
								myOLED.println(val);
							...
							else if (jsoneq(JSON_ptr, &JSON_t[i], "OUTP1") == 0) 
							{
								sprintf(buf,"%.*s", JSON_t[i+1].end-JSON_t[i+1].start,JSON_ptr + JSON_t[i+1].start);
								val=atoi(buf);
								myOLED.println(val);
								if(val==0)
								{
									ZIGBEE_Publish("OUTP1",0);
								}
								else
								{
									ZIGBEE_Publish("OUTP1",1);
								}
								i++;
							} 
							else if (jsoneq(JSON_ptr, &JSON_t[i], "OUTP2") == 0) 
							{
								sprintf(buf,"%.*s", JSON_t[i+1].end-JSON_t[i+1].start,JSON_ptr + JSON_t[i+1].start);
								val=atoi(buf);
								myOLED.println(val);
								if(val==0)
								{
									ZIGBEE_Publish("OUTP2",0);
								}
								else
								{
									ZIGBEE_Publish("OUTP2",1);
								}
								i++;
							} 
							...
							else if (jsoneq(JSON_ptr, &JSON_t[i], "LIGHT2") == 0) 
							{
								// We may want to do strtol() here to get numeric value 
								sprintf(buf,"%.*s", JSON_t[i+1].end-JSON_t[i+1].start,JSON_ptr + JSON_t[i+1].start);
								val=atoi(buf);
								myOLED.println(val);
								ZIGBEE_Publish("LIGHT2",val);
								i++;
							}
							else if (jsoneq(JSON_ptr, &JSON_t[i], "LIGHT3") == 0) 
							{
								// We may want to do strtol() here to get numeric value 
								sprintf(buf,"%.*s", JSON_t[i+1].end-JSON_t[i+1].start,JSON_ptr + JSON_t[i+1].start);
								val=atoi(buf);
								myOLED.println(val);
								ZIGBEE_Publish("LIGHT3",val);
								i++;
							}
						}
					}
