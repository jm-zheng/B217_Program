...
uint8_t read_y_8bit[] = {0x02,0x30,0x30,0x30,0x41,0x30,0x30,0x32,0x03,0x36,0x36};
...
uint8_t Set_M2[] = {0x02,0x31,0x30,0x31,0x30,0x30,0x30,0x31,0x30,0x34,0x03,0x42,0x41};
uint8_t Set_M3[] = {0x02,0x31,0x30,0x31,0x30,0x30,0x30,0x31,0x30,0x38,0x03,0x42,0x45};
uint8_t Set_M4[] = {0x02,0x31,0x30,0x31,0x30,0x30,0x30,0x31,0x31,0x30,0x03,0x42,0x37};
uint8_t read_m8_m10[] = {0x02,0x30,0x31,0x30,0x31,0x30,0x30,0x36,0x03,0x35,0x42};
...
void SET_M(uint8_t a )
{
	if(plc_stop == 0)
	{
	  ...
	  if(a == 0x31)
		{
		  for(int i=0;i<13;i++){RS232_Serial.write(Set_M1[i]);}
		}
		else if(a == 0x32)
		{
		  for(int i=0;i<13;i++){RS232_Serial.write(Set_M2[i]);}
		}
	  else if(a == 0x33)
		{
		  for(int i=0;i<13;i++){RS232_Serial.write(Set_M3[i]);}
		}
		else if(a == 0x34)
		{
		  for(int i=0;i<13;i++){RS232_Serial.write(Set_M4[i]);}
		}
...
//---------------shock_flag----------------------------------
	...
	//-----------------------------------------
		...
		if(second_flag < 999)
		{
		   ...
		   ampere_value = maxapval;
			
			//ampere_value = 0.0117*maxapval-0.276;
			ampere_value = 0.0111*maxapval-0.5656;
			if(ampere_value < 0)
			{
				ampere_value = 0 ;
			}
		...
		else if(second_flag == 1000)
		{
			...
			myOLED.print(RS232_RxBuf[2]);
			myOLED.print(RS232_RxBuf[3]);
			myOLED.println(RS232_RxBuf[4]);
			myOLED.print("sa1:");
			myOLED.print(maxapval);
			myOLED.println(ampere_value);
			/*myOLED.print("sc1:");
			myOLED.println(counts_counts);*/
			myOLED.print("sc1:");
			myOLED.print(counts_counts);
			
			
			//myOLED.println("*");
			ZIGBEE_Serial.write(0x06);
			ZIGBEE_Serial.write(0x06);
			ZIGBEE_Serial.write(0x06);
			...
			ZIGBEE_Serial.write(0x2c);
			ZIGBEE_Serial.write(RS232_RxBuf[2]);
			ZIGBEE_Serial.write(0x2c);
      ZIGBEE_Serial.write(RS232_RxBuf[3]);
			ZIGBEE_Serial.write(0x2c);
			ZIGBEE_Serial.write(RS232_RxBuf[4]);
			ZIGBEE_Serial.write(0x2c);
			ZIGBEE_Serial.print("sa1:");
			ZIGBEE_Serial.print(ampere_value);
			ZIGBEE_Serial.write(0x2c);
			ZIGBEE_Serial.print(maxapval);
			ZIGBEE_Serial.print(",ss1:");
			ZIGBEE_Serial.print(shock_counts);
		...
		while(ZIGBEE_Serial.available ())
			{
					...
					{
						 if (ZIGBEE_RxBuf[ZIGBEE_Count-3]==0x54 && ZIGBEE_RxBuf[ZIGBEE_Count-4] == 0x42)
						 {
